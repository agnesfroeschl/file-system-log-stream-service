...
copyFileDiffDir(){
    ...
    addLogEntry "copyDiffDir;""$fileToCopy"";""$targetDir"
...
copyFileSameDir(){
    ...
    addLogEntry "copySameDir;""$fileToCopy"";""$newDirAndFilename"
...
transformOperation(){
    ...
    do
        ...
        sleep $time
        echo $time
        echo "sleep" "'"$time"'" >> $eventScript
        time=$(echo "scale=2; "${time}"/2"| bc)
        st=`echo "$time > 0.0000" | bc`
        echo $st
    done
}
...
if [ -z "$1" ]
  then
    echo "No argument supplied - regular execution is 100 events performed"
